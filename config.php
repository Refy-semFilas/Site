...
function sendEmail($to, $subject, $body, $isHTML = true) {
    ...
    if (strpos(SITE_URL, 'localhost') !== false) {
        // Em modo desenvolvimento, salvar email em arquivo para debug
        $logFile = __DIR__ . '/email_log.html';
        $logContent = "<!-- Email enviado em: " . date('Y-m-d H:i:s') . " -->\n";
        $logContent .= "<!-- Para: $to -->\n";
        $logContent .= "<!-- Assunto: $subject -->\n";
        $logContent .= $body . "\n\n<hr>\n\n";
        
        file_put_contents($logFile, $logContent, FILE_APPEND | LOCK_EX);
        return true; // Simular envio bem-sucedido
    }
    
    return mail($to, $subject, $body, $headers);
}

// Função para gerar link de reset de senha
function generateResetLink($token) {
    return SITE_URL . '/Partedocliente/resetPasswordForm.html?token=' . $token;
}

// Função para criar o corpo do email de recuperação
    ...
    <body style='font-family: Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0; background-color: #f4f4f4;'>
        <div style='max-width: 600px; margin: 20px auto; background-color: white; border-radius: 10px; overflow: hidden; box-shadow: 0 0 20px rgba(0,0,0,0.1);'>
            ...
            <div style='background: linear-gradient(135deg, #ff6200, #ff9500); color: white; text-align: center; padding: 30px 20px;'>
                <h1 style='margin: 0; font-size: 28px;'>🍽️ Cantina System</h1>
            ...
            <div style='padding: 40px 30px;'>
                ...
                <p style='font-size: 16px;'>Recebemos uma solicitação para redefinir sua senha. Se você não fez esta solicitação, pode ignorar este email com segurança.</p>
                ...
                <div style='margin-top: 30px; padding: 20px; background-color: #fff3cd; border-radius: 8px; border-left: 4px solid #ffc107;'>
                    ...
                    <ul style='margin: 10px 0; padding-left: 20px; color: #856404; font-size: 14px;'>
                        <li>Este link expirará em <strong>" . TOKEN_EXPIRY_HOURS . " hora(s)</strong> por segurança</li>
            ...
            <div style='background-color: #f8f9fa; padding: 20px; text-align: center; border-top: 1px solid #dee2e6;'>
                <p style='margin: 0; font-size: 12px; color: #6c757d;'>
                    © " . date('Y') . " Cantina System. Todos os direitos reservados.<br>
...
// Função para registrar log de emails (para debug)
